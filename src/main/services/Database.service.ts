...
export function connect() {
  return Database(
    path.join( '/home/hafid/', 'clients.db'),
...
export function updateClient(client: CLIENT) {
  ...
  const stm = db.prepare(
    'UPDATE clients SET name = @name, card_number = @card_number, social_number = @social_number WHERE id = @id',
  );
...
export function insertCredit(credit: CREDIT) {
  ...
  const { id_client } = credit;
  const stm = db.prepare(
    'INSERT INTO credits (name, id_client, amount, date) VALUES (@name, @id_client, @amount, @date)',
  );
  const stm_client = db.prepare(
    'UPDATE clients SET total_amount = (SELECT COALESCE(SUM(amount), 0) FROM credits WHERE credits.id_client = @id_client ) WHERE EXISTS (SELECT 1 FROM credits WHERE credits.id_client = @id_client) AND clients.id = @id_client',
 );

  stm_client.run({ id_client})
  stm.run(credit);
}
...
export function updateCredit(credit: CREDIT) {
  const db = connect();
  const { id_client, amount, date, id } = credit;

  const stm = db.prepare(
    'UPDATE clients SET amount = @amount, id_client = @id_client, date = @date WHERE id = @id',
  );

  stm.run({ id_client, amount, date, id });

  const stm_client = db.prepare(
    'UPDATE clients SET total_amount = (SELECT COALESCE(SUM(amount), 0) FROM credits WHERE credits.id_client = @id_client ) WHERE EXISTS (SELECT 1 FROM credits WHERE credits.id_client = @id_client) AND clients.id_client = @id_client',
 );
...
export function deleteCredit(id: number, id_client: number) {
  const db = connect();

  const stm = db.prepare('DELETE FROM credits WHERE id = @id');

  const stm_client = db.prepare(
    'UPDATE clients SET total_amount = (SELECT SUM(amount) FROM credits WHERE credits.id_client = @id_client ) WHERE EXISTS (SELECT 1 FROM credits WHERE credits.id_client = @id_client) AND clients.id_client = @id_client',
 );
